...
from models.networks.DualReferenceIQANet import DualReferenceIQANet, DualReferenceIQANetNoMixer
...
class DRIQASolver(object):
    def __init__(self, config, args):
        ...
        # 模型
        self.DRNet = DualReferenceIQANetNoMixer(config, args)
        # 模型转设备
        self.DRNet = self.DRNet.to(self.device)
        # 取出DRNet中除了model_content和model_quality以外的参数other_params
        ...
        for t in range(self.epochs):
            ...
            if test_srcc + test_plcc + test_krcc > best_srcc + best_plcc + best_krcc:
                ...
                torch.save(self.DRNet.state_dict(), os.path.join(self.config.model_checkpoint_dir,
                                                                      f'{config.train_dataset}_in_saved_DRNet_NoMixer.pth'))
                print("DRNet更新：")
...
if __name__ == "__main__":
    ...
    # 保存控制台输出
    # TODO 路径修改:日志
    saver = Saver(f'./logs/In/{config.train_dataset}_b{config.batch_size}_lr{config.lr}in_NoMixer.log', sys.stdout)
    sys.stdout = saver
