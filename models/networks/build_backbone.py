...
from models.networks.resnet import model_dict
from models.networks.util import Normalize, JigsawHead
...
class RGBSingleHead(nn.Module):
    ...
    def __init__(self, name='resnet50', head='linear', feat_dim=128):
        ...
        dim_in = int(2048 * width)  # resnet的输出通道数
        self.width = width
...
def build_model(opt):
    # specify modal key
    branch = 'Mul' if opt.jigsaw else 'Sin'  # 表示投影头为单层还是多层
    model_key = opt.modal + branch  # modal默认为RGBSIN

    # arch 模型名; head 投影头; feat_dim 输出特征图通道数;
