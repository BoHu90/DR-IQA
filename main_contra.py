...
# 加载数据集并选择子集
dataset = KADISDataset(root_dir='/data/dataset/kadid10k/images', transform=transform)
print(f"Total number of samples in the dataset: {len(dataset)}")
subset_indices = list(range(10125))  # 选择一个包含10000个样本的子集
...
for epoch in range(200):  # 200个训练轮次
    for im_q, dist_type_q, dist_level_q, im_k, dist_type_k, dist_level_k in dataloader:
        ...
        model._dequeue_and_enqueue(k, dist_type_k)
