...
class DRIQASolver(object):
    def __init__(self, config, args):
        ...
        # 学习率\损失函数的定义
        # TODO 用AdamW(paras, weight_decay=0.01)
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            if test_srcc + test_plcc + test_krcc > best_srcc + best_plcc + best_krcc:
                ...
                torch.save(self.DRNet.state_dict(), os.path.join(self.config.model_checkpoint_dir,
                                                                      f'{config.train_dataset}_cross_{config.test_dataset}_b{config.batch_size}_lr{config.lr}_{config.mlp_depth}depth_{config.model}.pth'))
                print(f"{config.train_dataset}_trained_DRNet更新：")
        ...
        # 取所有epoch最佳的指标
        # 库内
        print('Best %s cross %s test SRCC %f, PLCC %f, KRCC %f\n' % (config.train_dataset, config.test_dataset, best_srcc, best_plcc, best_krcc))
...
if __name__ == "__main__":
    ...
    # 保存控制台输出
    # TODO 路径修改:日志
    saver = Saver(f'./logs/Cross/{config.train_dataset}_cross_{config.test_dataset}_b{config.batch_size}_lr{config.lr}_{config.mlp_depth}depth_{config.model}.log', sys.stdout)
    sys.stdout = saver
